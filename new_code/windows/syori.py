...
def Syori(clause_list,clause_num,clause,negative_choice,frag):
    """
    ...
    for value in clause:
        ...
        for i in range(start_pos,end_pos + 1):
            ...
            #--------------------------------------
            #文節の単位で情報の抽出
            if not re.findall(ur"\* \dD|\* \dP|\* -\dD|\* \dP",sentence) == []:
                #--------------------------------------------------
                #並列構造に関する記述
                #<並キ:名>は並列構造の存在を示すタグ ただ、:名　が何を示すのか？は不明

                if not re.findall(ur"<並キ:.*?>",sentence) == []:
                    parallel_type = "".join((re.findall(ur"<並列タイプ:.*?>",sentence))).replace(u"<並列タイプ:","").strip(">")
                    bnst_dic["parallel_type"] = parallel_type

                    #並列の句がいくつ続いているのか？
                    parallel_p_num = "".join((re.findall(ur"<並結句数:.*?>",sentence))).replace(u"<並結句数:","").strip(">")
                    bnst_dic["parallel_p_num"] = parallel_p_num

                    #並列の文節がいくつ続いているのか？
                    parallel_c_num = "".join((re.findall(ur"<並結文節数:.*?>",sentence))).replace(u"<並結文節数:","").strip(">")
                    bnst_dic["parallel_c_num"] = parallel_c_num


                if not re.findall(ur"\* \dD|\* -\dD|\* \dP|\* -\dP",sentence) == []:
                    bnst_dependency = re.findall(ur"\* \d|\* -\d",sentence)
                ...
                #節の種類に関する記述
                c_clause_type = ""
                if not re.findall(ur"<主節>",sentence) == []:
                    c_clause_type = "".join(re.findall(ur"<主節>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})
                if not re.findall(ur"<連用節>",sentence) == []:
                    c_clause_type = "".join(re.findall(ur"<連用節>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})
                if not re.findall(ur"<連体節>",sentence) == []:
                    c_clause_type = "".join(re.findall(ur"<連体節>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})

                bnst_dic["c_clause_type"] = c_clause_type

                #--------------------------------------------------
                #数量に関係する記述（基本句と共通）
                bnst_dic["c_counter"] = "".join(re.findall(ur"<カウンタ:.*?>",sentence)).replace(u"<カウンタ:","").strip(">")

                if not re.findall(ur"<数量>",sentence) == []:
                    bnst_dic["c_kazu"] = "yes"
            ...
            if not re.findall(ur"\+ \dD|\+ \dP|\+ -\dD|\+ -\dP",sentence) == []:
                ...
                # windows 版KNPはどうしてがか、述語情報がでない。よって以下の記述はwindows版では無効
                predicate = ""

                if not re.findall(ur"<状態述語>",sentence) == []:

                    predicate = re.findall(ur"<状態述語>",sentence)
                if not re.findall(ur"<動態述語>",sentence) == []:

                    predicate = re.findall(ur"<動態述語>",sentence)
                if not re.findall(ur"<体言止>",sentence) == []:

                    predicate = re.findall(ur"<体言止>",sentence)

                if not predicate == "":
                    predicate = "".join(predicate).translate({ord(u'<') : None, ord(u'>') : None})
                ...
                #動詞の時制について
                if not re.findall(ur"<時制.*?>",sentence) == []:
                    t_c = "".join(re.findall(ur"<時制.*?>",sentence))
                    tense = t_c.replace(u"時制-","").translate({ord(u'<') : None, ord(u'>') : None})
                ...
                main_case = re.findall(ur"<主題格:.*?>",sentence)
                main_case = "".join(main_case).replace(u"<主題格:","").strip(">")

                bnst_dic["main_case"] = main_case
                #<文末>タグについて
                position_t = re.findall(ur"<文末>",sentence)
                ...
                if not re.findall(ur"<並キ:.*?>",sentence) == []:
                    ...
                    bnst_dic["para_type"] = "".join(re.findall(ur"<並列タイプ:.*?>",sentence)).replace(u"<並列タイプ:","").strip(">")
                ...
                clause_type = ""
                if not re.findall(ur"<主節>",sentence) == []:
                    clause_type = "".join(re.findall(ur"<主節>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})
                if not re.findall(ur"<連用節>",sentence) == []:
                     clause_type = "".join(re.findall(ur"<連用節>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})
                if not re.findall(ur"<連体節>",sentence) == []:
                    clause_type = "".join(re.findall(ur"<連体節>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})

                bnst_dic["clause_type"] = clause_type
                #--------------------------------------------------
                #数量に関係する記述（文節と共通）
                bnst_dic["k_counter"] = "".join(re.findall(ur"<カウンタ:.*?>",sentence)).replace(u"<カウンタ:","").strip(">")
                ...
                #その語が形容詞で用言の場合
                modify_type = ""
                if not re.findall(ur"<用言:形>",sentence) == []:
                    modify_type = "".join(re.findall(ur"<用言:形>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})
                #「おおきくて」のような場合、形容詞でなく、連体詞となる
                if not re.findall(ur"<連体修飾>",sentence) == []:
                    modify_type = "".join(re.findall(ur"<連体修飾>",sentence)).translate({ord(u'<') : None, ord(u'>') : None})

                #別に条件は「副詞」だけでもいいんだろうが、念のため
                if not re.findall(ur"<副詞>",sentence) == [] and not re.findall(ur"<修飾>",sentence) == []:
                    modify_type = u"副詞"
                ...
                #節機能に関する記述
                bnst_dic["clause_func"] = "".join(re.findall(ur"<節機能-.*?>",sentence)).replace(u"<節機能-","").strip(">")
                ...
                for para in i_tmp_list:
                    para = para.replace(">","")
                    if not re.findall(ur"モダリティ",para) == []:
                        para = para.replace(u"モダリティ-","")
                        modality_list.append(para)

                    if not re.findall(ur"敬語:.*",para) == []:
                        para = para.replace(u"敬語:","")
                        respect = para

                    if not re.findall(ur"格関係.*",para) == []:
                        para = re.sub(ur"格関係\d:","",para)
                        case_relation_list.append(para)

                        bnst_dic["case_relation"] = case_relation_list

                    if not re.findall(ur"解析格:.*",para) == []:
                        kaiseki_case = para.replace(u"解析格:","").strip("\n")

                        bnst_dic["kaiseki_case"] = kaiseki_case

                #----------------------------------------------------
                #格に関する情報の獲得
                if not re.findall(ur"<格要素>",sentence) == []:
                ...
                if not re.findall(ur"<主題表現>",sentence) == []:
                    bnst_dic["main"] = "yes"

                    if not re.findall(ur"私",sentence) == []:
                        bnst_dic["per"] = 1

                    if not re.findall(ur"あなた",sentence) == []:
                        bnst_dic["per"] = 2
                ...
                #--------------------------------------
                #正規化表記を拾う
                reg_exp = re.findall(ur"(<正規化代表表記:.*?>)",sentence)
                if not reg_exp == []:
                    split_list = []
                    split_list = "".join(reg_exp).split("/")
                    split_list[0] = (split_list[0]).replace(u"<正規化代表表記:","")
                    split_list[1] = (split_list[1]).replace(">","")
                    bnst_dic["morp"] = split_list
                    #形態素。これを形態素の抽出部に渡す
                    reg_marker = re.findall(ur"(<正規化代表表記:.*?>)",sentence)

                #--------------------------------------
                #基本句単位でのかかりうけ情報を獲得
                if not re.findall(ur"\+ \dD|\+ -\dD|\+ \dP|\+ -\dP",sentence) == []:
                    bnst_dic["k_dep"] = re.findall(ur"\+ \d|\+ -\d",sentence)
            ...
            #--------------------------------------
            #形態素情報
            if re.findall(ur"\+ \dD|\+ \dP|\+ -\dD|\+ -\dP",sentence) == [] and re.findall(ur"\* \dD|\* \dP|\* -\dD|\* -\dP",sentence) == []:
                ...
                reg_marker_morp = re.findall(ur"(<正規化代表表記:.*?>)",sentence)

                bnst_dic["dom"] = "".join(re.findall(ur"<ドメイン:.*?>",sentence)).replace(u"<ドメイン:","").strip(">")
                bnst_dic["cat"] = "".join(re.findall(ur"<カテゴリ:.*?>",sentence)).replace(u"<カテゴリ:","").strip(">")
    ...
    #ここから、出力チェック用の記述
    if frag == 1:
        print u"About OutPut list"
        print u"Contents of list is:",out_list
        for one in out_list:
            print u"---------------------------------------"
            print u"input_morpheme is:",one.input_morp
            print u"regular morpheme is:",one.reg_morp_form
            print u"pos is:",one.pos
            print u"tense is:",one.tense
            print u"bnst clause type is:",one.c_clause_type
            print u"kihon clause type is:",one.clause_type
            print u"kihon next dependenct number:",one.k_dep
            print u"kihon position number:",one.k_position
            print u"kihon clause function is:",one.clause_func
            print u"mofifier or not:",one.modify_check
            print u"case or not:",one.case_check
            print u"main case is:",one.main_case
            print u"main:",one.main
            print u"predicate or not:",one.predicate_check
            print u"kaiseki case:",one.kaiseki_case
            print u"case relation is:",one.case_relation
            for one_one in one.case_relation:
                print one_one
            print u"para_check is:",one.para_check
            print u"para range(kihon pharase) is:",one.p_p_num
            print u"para range(clause) is \n*note* includes this node and dependent node:",one.p_c_num
            print u"para type is:",one.para_type
            print u"counter check(kihon)",one.k_kazu
            print u"counter type(kihon)",one.k_counter
    #--------------------------------------
