...
def make_entry_dic(sent):
    ...
    #KNPの解析処理過程のエラー，syori.pyのエラーなどが出た場合はその後の処理を中止
    if out_list == u'error':
        return u'error'
...
def overlap_key(entry_dic,prev_key,i):
    ...
    if frag == 1:
        for e_key in overlap_dic:
            print 'overlap_dic key:',e_key.encode('utf-8')
...
def load_sentence():
    ...
    if os.path.exists('./sentence-list'):
        ...
        for sent_i,sent in enumerate(sent_list):
            ...
            if entry_dic == u'error':
                print sent,'\n'
                error_counter += 1
                error_line_list.append(sent)
                continue
